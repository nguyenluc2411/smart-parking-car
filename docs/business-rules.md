...
- **BR-009-3 (Giữ chỗ phải là chỗ thật):** Đặt chỗ **lấy một slot thật ra khỏi pool** và chuyển nó sang `RESERVED`. Một lượt đặt không rút slot khỏi pool thì không phải đặt chỗ — tài xế đến nơi thấy chỗ đã có xe, tệ hơn là chưa từng có tính năng này. Slot được chọn (tự động hoặc do tài xế chỉ định, BR-009-10) đều **row-locked**, nên hai tài xế tranh chỗ cuối cùng không thể cùng được cấp.
- **BR-009-10 (Tài xế tự chọn zone + ô):** `GET /api/v1/driver/slots` trả toàn bộ slot (`slotCode`, `zone`, `gridRow`, `gridCol`, `status` — KHÔNG kèm `currentSessionId` hay bất kỳ định danh phiên/biển nào của xe khác) để app vẽ bản đồ thật theo zone, không phải sơ đồ giả lập. `POST /api/v1/driver/reservations` nhận thêm `slotId` (tùy chọn): nếu có, server khóa **đúng slot đó** (row-locked, cùng cơ chế BR-009-3) và chỉ chấp nhận nếu nó còn `EMPTY` tại thời điểm giữ — **409** nếu ai đó vừa lấy mất (bản đồ tài xế xem có thể đã cũ vài giây). Không gửi `slotId` thì giữ hành vi cũ: server tự chọn slot `EMPTY` đầu tiên.
  - Đổi hướng có chủ đích so với thiết kế ban đầu của BR-009 (server luôn tự chọn để không lộ sơ đồ bãi/tránh script vợt chỗ): giờ chấp nhận đánh đổi đó để tài xế thấy trống/đầy và chủ động chọn vị trí. Giảm nhẹ rủi ro vợt chỗ bằng cách **không** lộ danh tính người đang đậu (chỉ trạng thái ô, không có biển/phiên), và giữ nguyên chống race-condition bằng row lock.
- **BR-009-11 (Phí đặt chỗ — chặn spam, tối ưu doanh thu):** `POST /api/v1/driver/reservations/{id}/fee` (billing-service) tạo một khoản phí = `reservation-fee.percent-of-min-charge` (mặc định **30%**) × `min_charge` của rate đang hiệu lực, thanh toán qua PayOS (link/QR có sẵn, tái dùng gateway của hóa đơn). Không bắt buộc để đặt chỗ thành công (`create` reservation không phụ thuộc fee), nhưng driver app luôn hỏi thanh toán ngay sau khi giữ chỗ.
  - **Hủy sớm ≥ `refund-cutoff-minutes` (mặc định 30 phút) trước `startTime` → hoàn 100%** (`REFUNDED`). Hủy sát giờ hơn hoặc bỏ hẹn (`EXPIRED`) → **mất phí** (`FORFEITED`). Phí chưa thanh toán (`PENDING`) lúc hủy thì hủy luôn link thanh toán, không tính là mất gì.
  - Bảng `reservation_fees` (billing_db) tách riêng khỏi `payments`/`invoices` — `reservation_id` là tham chiếu lỏng sang parking-service (Database Per Service, không FK xuyên service); client cung cấp `plateNumber`/`reservationStartTime` vì billing-service không đọc trực tiếp DB của parking-service.
  - **Hoàn tiền hiện là mock**: hệ thống tự cập nhật trạng thái `REFUNDED` trong DB của mình, **không** gọi API hoàn tiền thật của PayOS/MoMo (chưa tích hợp — cùng mức độ mock với luồng driver tự thanh toán online hiện có). Tích hợp hoàn tiền thật qua cổng thanh toán là việc lớn hơn, cần ADR riêng nếu triển khai thật.
...
- **BR-009-8 (Bỏ hẹn nhiều lần):** Biển có ≥ `no-show-limit` lượt `EXPIRED` (mặc định **3**) trong `no-show-window-days` (mặc định **30 ngày**) bị **tạm khóa quyền đặt chỗ**. Người giữ chỗ rồi không đến đang chặn chỗ của khách sẵn sàng trả tiền, nên phải có cái giá.
  - **Cập nhật (BR-009-11): quyết định "không thu tiền cọc" ở trên đã được xem lại** — nay có thu một khoản phí đặt chỗ nhỏ, tách biệt khỏi luồng hoàn tiền/tranh chấp mà quyết định gốc lo ngại (xem BR-009-11). Cơ chế khóa-tài-khoản-3-lần vẫn giữ nguyên, dùng song song với khoản phí.
- **BR-009-9 (Chưa tính tiền giữ chỗ vào hóa đơn gửi xe):** Hóa đơn vẫn tính từ `entry_time` như mọi xe khác (BR-004-5) — thời gian giữ chỗ trước khi xe vào **không** cộng vào tiền gửi xe. Phí đặt chỗ (BR-009-11) là một khoản **riêng biệt**, không phải tiền gửi xe trả trước.
