...
const METHOD_LABEL: Record<MethodRevenue["method"], string> = {
  CASH: "Tiền mặt",
  CASH_OFFLINE: "Tiền mặt (mất điện)",
...
const METHOD_COLOR: Record<MethodRevenue["method"], string> = {
  ...
  // Same family as CASH — it is cash — but distinct, because outage takings are the rows an
  // operator has to reconcile against paper vouchers (BR-005-7).
  CASH_OFFLINE: "hsl(var(--success) / 0.55)",
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="rounded-xl border border-border/50 bg-background/95 p-3.5 shadow-xl backdrop-blur-md min-w-[160px]">
        ...
        <p className="text-base font-bold tabular-nums text-foreground">{formatCurrency(d.amount)}</p>
...
const PaymentMethodChartComponent = ({ data }: { data: MethodRevenue[] }) => {
  const withRevenue = data.filter((d) => d.amount > 0);
